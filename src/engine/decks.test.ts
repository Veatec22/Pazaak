...
const ALLOWED = new Set(['plus', 'minus', 'flip']);

describe('player pools', () => {
  it('contain only plus/minus/flip families', () => {
    for (const cards of Object.values(POOLS)) {
      expect(cards.every((c) => ALLOWED.has(c.family))).toBe(true);
    }
  });

  it('have the expected sizes', () => {
    expect(POOLS.classic).toHaveLength(12);
    expect(POOLS.flip).toHaveLength(6);
...
describe('player deck', () => {
  ...
  it('deckFromPool only yields allowed families', () => {
    const rng = new SeededRng(5);
    for (let i = 0; i < 100; i++) {
      for (const c of deckFromPool(rng, 'mix')) expect(ALLOWED.has(c.family)).toBe(true);
    }
  });
});

describe('campaign AI tiers', () => {
  it('has 5 tiers, each a legal 10-card deck of allowed families', () => {
    expect(CAMPAIGN_LENGTH).toBe(5);
    CAMPAIGN_TIERS.forEach((_, i) => {
      const deck = aiDeck(i);
      expect(deck).toHaveLength(SIDE_DECK_SIZE);
      expect(deck.every((c) => ALLOWED.has(c.family))).toBe(true);
    });
  });
  ...
  it('ramps up flip density from VeryEasy to VeryHard', () => {
    ...
    expect(flips[0]).toBe(0);
    expect(flips[4]).toBe(10);
